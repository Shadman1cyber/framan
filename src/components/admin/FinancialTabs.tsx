            ...
            <ChartCard title="شاخص‌ها" icon={<ListIcon />}>
              <KeyValueList
                items={[
                  ...
                  { label: "تخفیف امروز", value: `${formatToman(summary.discounts?.today ?? 0)} (${formatNumber(summary.discounts?.ordersWithDiscount.today ?? 0)} سفارش)` },
                  { label: "تخفیف ماه", value: `${formatToman(summary.discounts?.month ?? 0)} (${formatNumber(summary.discounts?.ordersWithDiscount.month ?? 0)} سفارش)` },
                ]}
              />
            </ChartCard>
          </div>

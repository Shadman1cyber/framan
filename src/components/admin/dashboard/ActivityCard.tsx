...
/** Business-wide activity feed; per-item dots keep their status colors. */
export function ActivityCard() {
  return <section className="flex h-[230px] flex-col rounded-[14px] border border-dashboard-line bg-dashboard-surface/75 p-4"><div className="mb-5 flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-semibold text-dashboard-foreground"><span className="module-accent-text"><ClockIcon /></span>آخرین فعالیت‌ها</div><Link href="/admin/orders" aria-label="مشاهده فعالیت‌ها" className="text-dashboard-muted transition-colors hover:text-dashboard-foreground">←</Link></div><ol className="divide-y divide-dashboard-line/60">{activities.map((item) => <li key={`${item.time}-${item.text}`} className="flex items-center gap-2.5 py-1 first:pt-0 last:pb-0"><span className="w-11 shrink-0 font-mono text-[11px] leading-relaxed text-dashboard-muted">{item.time}</span><span className="min-w-0 flex-1 truncate text-[11px] leading-relaxed text-dashboard-foreground"><span className="block truncate">{item.text}</span><span className="block truncate text-[11px] leading-3 text-dashboard-muted">{item.detail}</span></span><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${item.tone}`} aria-hidden="true" /></li>)}</ol></section>;
}
